...
export function SelectField({ label, hint, value, options, favorites, disabledSet, selfValue, onChange, emptyLabel }) {
  ...
  // Liste filtrée pour le menu déroulant. On garde toujours la valeur choisie visible.
  const filtered = q
  ...
  return (
    <div>
      ...
      {/* Champ de recherche pour filtrer (affiché s'il y a beaucoup de coureurs) */}
      {options.length > 6 && (
        <input
          type="text"
          value={query}
      ...
      {/* Menu déroulant natif : marche bien partout, surtout sur mobile */}
      <select
        value={value || ""}
      ...
      >
        ...
        {filtered.map((r) => (
          <option key={r} value={r} disabled={isDisabled(r)}>
      ...
      {q && filtered.length > 0 && (
        <div className="mono text-[10px] text-stone-400 mt-1">
          {filtered.filter((r) => r !== value || !q).length} coureur(s) — ouvre la liste pour choisir
        </div>
      )}
    </div>
  );
